...
include Nickbot::STATUS

module Nickbot
  # Main robot class. Interprets string commands. Returns responses
  # @author Nick Butler
  # @attr [Nickbot::Table] table the table the robot will be placed upon
  # @attr_reader [String] status (Nickbot::STATUS) describing the robot's state
  # @attr_reader [Nickbot::Placement] placement describes the robot's current
  #   position
  class Robot
    include STATUS
    ...
    # @private
    # @return [String] generated from (Nickbot::COMMANDS)
    def help
    ...
    # @param [String] orientation from (Nickbot::ORIENTATIONS)
    # @return [String, nil] (Nickbot::COMMAND_SUCCESSFUL) or nil if error
    ...
    # @return [String, nil] (Nickbot::COMMANDS::COMMAND_SUCCESSFUL) or nil if
    #   error
    def left
      return unless check_placement
      placement.turn_left
    ...
    # Report the robots current position on it's (Nickbot::Table)
    # @return [String] ascii depiction of (Nickbot::Table) with robot's current
    ...
    #   Nickbot::STATUS::NOT_PLACED if not placed
    def check_placement
    ...
    # @retrn [String] (Nickbot::STATUS::UNRECOGNISED_COMMAND)
